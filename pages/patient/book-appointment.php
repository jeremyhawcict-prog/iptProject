...
<div class="card-glass" style="padding:32px;max-width:800px;margin:0 auto;">
  ...
  <div id="wizardStep2" style="display:none;">
    <div id="selectedDoctorInfo" style="background:var(--input-bg,rgba(232,244,251,.55));border-radius:12px;padding:16px;margin-bottom:20px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;"></div>

    <!-- Working days indicator -->
    <div id="workingDaysBar" style="margin-bottom:16px;display:none;">
      <p style="font-size:.8rem;font-weight:600;color:var(--text-muted,#64748B);margin:0 0 8px;text-transform:uppercase;letter-spacing:.05em;">
        <i class="fa-solid fa-calendar-check" style="color:var(--teal-core,#3D6A8A);"></i> Doctor's Working Days
      </p>
      <div id="workingDaysPills" style="display:flex;flex-wrap:wrap;gap:6px;"></div>
    </div>

    <div class="form-group">
      <label class="form-label">Select Date</label>
      <input type="date" id="apptDate" class="form-control" min="<?= date('Y-m-d') ?>" style="max-width:260px;" />
      <p id="dateHint" style="font-size:.78rem;color:var(--text-muted,#64748B);margin:6px 0 0;display:none;">
        <i class="fa-solid fa-circle-info"></i> Please choose one of the working days shown above.
      </p>
    </div>
    <div class="form-group">
...
/* Working-day pills */
.day-pill{display:inline-flex;align-items:center;padding:4px 12px;border-radius:20px;font-size:.78rem;font-weight:600;}
.day-pill.active{background:rgba(16,185,129,.12);color:#059669;border:1px solid #10B981;}
.day-pill.inactive{background:rgba(100,116,139,.09);color:var(--text-muted,#64748B);border:1px solid rgba(100,116,139,.2);text-decoration:line-through;opacity:.6;}
/* Shake animation for invalid date selection */
...
(function(){
  ...
  /* Days of week ordered Mon-Sun */
  var ALL_DAYS = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];

  /**
   * Returns the doctor's available_days as a normalised array of full day names.
   * Handles both raw JSON strings (legacy) and already-parsed arrays.
   */
  function getAvailableDays(doctor) {
    if (!doctor) return [];
    var raw = doctor.available_days;
    if (!raw) return [];
  ...
  /**
   * Returns true if dateStr (YYYY-MM-DD) falls on one of availDays.
   * If availDays is empty every day is valid (doctor has no restriction set).
   */
  function isDayAllowed(dateStr, availDays) {
    if (!availDays || availDays.length === 0) return true;
    // Append T00:00:00 to force local-midnight parse (avoids UTC off-by-one)
    var date = new Date(dateStr + 'T00:00:00');
    var dayName = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][date.getDay()];
    return availDays.indexOf(dayName) !== -1;
  }

  /** Renders the working-days pill strip and date hint text. */
  function renderWorkingDaysBar(availDays) {
    var bar   = document.getElementById('workingDaysBar');
    var pills = document.getElementById('workingDaysPills');
    var hint  = document.getElementById('dateHint');

    if (!availDays || availDays.length === 0) {
    ...
    ALL_DAYS.forEach(function(day) {
      var isActive = availDays.indexOf(day) !== -1;
      var pill = document.createElement('span');
      pill.className = 'day-pill ' + (isActive ? 'active' : 'inactive');
      pill.innerHTML = (isActive ? '<i class="fa-solid fa-check" style="font-size:.7rem;margin-right:3px;"></i>' : '') + day.substring(0, 3);
      pills.appendChild(pill);
    });
  }

  /** Centralised handler for when a doctor is selected (list click or URL param). */
  function onDoctorSelected(d) {
    selectedDoctor = d;
    var availDays  = getAvailableDays(d);
    ...
    renderWorkingDaysBar(availDays);

    // Reset date + slots on every doctor switch
  ...
  function showStep(n) {
    ...
    for (var i = 1; i <= 3; i++) {
      document.getElementById('stepInd' + i).classList.toggle('active', i <= n);
    }
  }
  ...
  function loadDoctors() {
    ...
    utils.apiGet(utils.apiUrl('doctors/list.php'), params, function(err, data) {
      ...
      if (!data || !data.success || !data.data.doctors || !data.data.doctors.length) {
        el.innerHTML = '<p class="text-muted" style="text-align:center;grid-column:1/-1;padding:20px;">No doctors found.</p>';
        return;
      }
      el.innerHTML = '';
  ...
  // Auto-select doctor from URL param ?doctor=<id>
  if (preselectedDoctorId) {
    utils.apiGet(utils.apiUrl('doctors/profile.php'), { id: preselectedDoctorId }, function(err, data) {
      if (data && data.success && data.data.doctor) {
        onDoctorSelected(data.data.doctor);
      }
    });
  }

  // Date change: validate day-of-week, then load slots
  document.getElementById('apptDate').addEventListener('change', function() {
    if (!selectedDoctor) return;

    var availDays = getAvailableDays(selectedDoctor);

    // Enforce working-day restriction
    if (availDays.length > 0 && !isDayAllowed(this.value, availDays)) {
      var input   = this;
      ...
      utils.showAlert(
        '<i class="fa-solid fa-calendar-xmark"></i> <strong>' + dayName + '</strong> is not a working day for Dr. '
        + utils.escapeHtml(selectedDoctor.full_name)
        + '. Available days: <strong>' + availDays.join(', ') + '</strong>.',
        'warning'
      );
    ...
    // Valid day — fetch available slots
    ...
    utils.apiGet(utils.apiUrl('doctors/availability.php'), { doctor_id: selectedDoctor.id, date: this.value }, function(err, data) {
      if (!data || !data.success || !data.data.slots || !data.data.slots.length) {
        slotsEl.innerHTML = '<span class="text-muted text-sm">No slots available for this date.</span>';
        return;
      }
      slotsEl.innerHTML = '';
      data.data.slots.forEach(function(s) {
        ...
        btn.addEventListener('click', function() {
          ...
          utils.apiPost(utils.apiUrl('appointments/slots/reserve.php'), { slot_id: s.slot_id }, function() {
  ...
  document.getElementById('confirmBooking').addEventListener('click', function() {
    ...
    utils.apiPost(utils.apiUrl('appointments/book.php'), {
      ...
      if (data && data.success) {
        ...
        utils.showAlert(data ? data.message : 'Booking failed.', 'error');
      }
    });
  });
})();
